...
function getOriginMoviestest(n){//根据心情mood获得n部电影
    $.ajax({
        type: "get",
        url: "http://192.168.2.113:3000/daily?num="+n+"&mood=" + mood,
...
function addInfo(origin,n){//n是每页显示的电影数，从origin开始
    ...
    for (var i = origin,j=0; i < origin+n; i++,j++){
        ...
        if (movieContent.length>89)
            movieContent = movieContent.slice(0,86)+"...";
